...
public class ColorServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (dto.getId() == 0) {
			...
			boolean b = ser.insertColor(dto, request, config);

			if (b) {

				response.sendRedirect("add_color.jsp?msg=Yes");
			} else {

				response.sendRedirect("add_color.jsp?msg=No");

			}
		}

		else {
			boolean b = ser.UpdateColor(dto, request, config);

			if (b) {

				response.sendRedirect("manage_color.jsp?msg=YesUp");
			} else {

				response.sendRedirect("manage_color.jsp?msg=NoUp");
