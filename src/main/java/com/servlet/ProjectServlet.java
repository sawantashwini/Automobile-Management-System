...
public class ProjectServlet extends HttpServlet {
			...
			while (itr.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if (name.equals("Mobile_no"))
						dto.setMobile_no(item.getString() == null ? "0"
								: item.getString().trim());
					if (name.equals("Local_address"))
						dto.setLocal_address(item.getString() == null ? "" : item
								.getString().trim());
					
					if (name.equals("Gst_in"))
						dto.setGst_in(item.getString() == null ? "0"
								: item.getString().trim());
					...
					if (name.equals("Term_and_conditions"))
						dto.setTerm_and_conditions(item.getString() == null ? "" : item
								.getString().trim());
					...
					if (item.getSize() != 0) {
						if (item.getSize() < 3000000) {
							if (dto.getId() == 0) {
								...
								savesFile = new File(path + "ProjectImage/" + j
										+ ".jpg");
								...
								savesFile = new File(path + "ProjectImage/"
										+ dto.getId() + ".jpg");
								try {
									item.write(savesFile);
	...
	if(dto.getId()==0){
		...
		if (b) {
			...
			response.sendRedirect("add_project.jsp?msg=Yes");
		} else {

			response.sendRedirect("add_project.jsp?msg=No");
