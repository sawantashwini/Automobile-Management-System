...
import com.service.SaleBillService;

/**
 * Servlet implementation class SellBillServlet
 */
@WebServlet("/SaleBillServlet")
public class SaleBillServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		dto.setTotal_basic_amt(Float.parseFloat(
				request.getParameter("Total_basic_amt") == null ? "0" : request.getParameter("Total_basic_amt")));

		dto.setDiscount_per(Float.parseFloat(
				request.getParameter("Discount_per") == null ? "0" : request.getParameter("Discount_per")));

		dto.setPaid_amount(Float
				.parseFloat(request.getParameter("Paid_amount") == null ? "0" : request.getParameter("Paid_amount")));

		dto.setCash_amount(Float
				.parseFloat(request.getParameter("Cash_amount") == null || request.getParameter("Cash_amount") == "" ? "0" : request.getParameter("Cash_amount")));
		...
		dto.setTotal_gst_amt(Float.parseFloat(
				request.getParameter("Total_gst_amt") == null ? "0" : request.getParameter("Total_gst_amt")));

		dto.setTotal_amt_with_gst(Float.parseFloat(request.getParameter("Total_amt_with_gst") == null ? "0"
				: request.getParameter("Total_amt_with_gst")));

		dto.setDiscount_amount(Float.parseFloat(
				request.getParameter("Discount_amount") == null ? "0" : request.getParameter("Discount_amount")));

		dto.setFinal_amount(Float.parseFloat(
				request.getParameter("Final_amount") == null ? "0" : request.getParameter("Final_amount")));
		...
		dto.setGst_amount_5(Float.parseFloat(
				request.getParameter("Gst_amount_5") == null ? "0" : request.getParameter("Gst_amount_5")));
		...
		pay_dto.setOnline_amount(Float.parseFloat(
				request.getParameter("Online_amount") == null || request.getParameter("Online_amount") == "" ? "0" : request.getParameter("Online_amount")));
		pay_dto.setCash_amount(Float
				.parseFloat(request.getParameter("Cash_amount") == null || request.getParameter("Cash_amount") == "" ? "0" : request.getParameter("Cash_amount")));
		// String
		pay_dto.setRemark(request.getParameter("Remark") == null ? "N/A" : request.getParameter("Remark"));
		...
		if (dto.getId() == 0) {
			...
			id = ser.insertSaleBillInfo(pay_dto, dto, request, config);

			b = ser.insertSaleBillItem(Sell_base_price, Item_id, Cat_id, Measure_id, Cat_name, Measure_name,
					Hsn_code, Gst_per, Item_name, Item_code, Qty, Tot_amt, Dis_per, Dis_amt, Discount_sell_gst_price,
					Sell_item_status,Warranty, dto, id, request, config);
			if (id>0) {

				response.sendRedirect("print_sell_bill.jsp?msg=Yes&id=" + id);

			} else {

				response.sendRedirect("sell_bill.jsp?msg=No");
			}
		}
		...
		// condition for updating
			...
			b = ser.updateSaleBillInfoById(dto, pay_dto, request, config);

			if (b) {

				response.sendRedirect("print_sell_bill.jsp?msg=YesUp&id=" + dto.getId());
			} else {

				response.sendRedirect("manage_sell_bill.jsp?msg=NoUp");
