 ...
 */
@WebServlet("/PurchaseBillServlet")
public class PurchaseBillServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (dto.getId() == 0) {
			...
			id = ser.insertPurchaseBillInfo(dto, request, config);

			boolean b = ser.insertPurchaseBillItem(Pur_base_price, Item_id, Cat_id, Measure_id, Cat_name, Measure_name,
					Hsn_code, Gst_per, Item_name, Item_code, Qty, Tot_amt, Dis_per, Dis_amt, Pur_discount_price,
					Pur_item_status, dto, id, request, config);
			if (id > 0) {

				response.sendRedirect("print_purchase_bill.jsp?msg=Yes&id=" + id);

			} else {

				response.sendRedirect("purchase_bill.jsp?msg=No");
			}
		}
		...
		// condition for updating
			...
			boolean b = ser.updatePurchaseBillInfoById(dto, request, config);

			if (b) {

				response.sendRedirect("print_purchase_bill.jsp?msg=YesUp&id=" + dto.getId());

			} else {

				response.sendRedirect("manage_purchase_bill.jsp?msg=NoUp");
