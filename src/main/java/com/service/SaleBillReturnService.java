...
public class SaleBillReturnService {
	...
	public String getMaxInvoiceNo(String s, HttpServletRequest request, ServletConfig config) throws IOException {
		...
		try {
			...
			preparedStatement = comm.connection.prepareStatement(
					"SELECT MAX(CAST(SUBSTRING(invoice_no, 1, LENGTH(invoice_no)-0) AS UNSIGNED)) FROM sale_bill_return_tb");
	...
	PaymentService pay_ser = new PaymentService();
		...
		public int insertSaleBillReturnInfo(PaymentDto pay_dto,SaleBillDto dto, HttpServletRequest request, ServletConfig config)
				throws IOException {
			...
			try {
				...
				PreparedStatement ps = db.connection.prepareStatement("\r\n"
						+ "INSERT INTO sale_bill_return_tb (invoice_no,customer_id_fk,c_y_session,total_basic_amt,total_gst_amt,total_amt_with_gst,discount_per, \r\n"
						...
						+ "online_remark,online_way,gst_status,remark,igst_status,bank_id_fk)\r\n"
				 ...
				 * 
				 * if(i!=0){ // update customer due info PreparedStatement cust_due =
				 * db.connection .prepareStatement( "UPDATE customer_info_tb SET \n" +
				 * "	old_due = old_due + ? WHERE id = ?;");
				 * 
				 * cust_due.setFloat(1,
				 * dto.getFinal_amount()-(dto.getCash_amount()+dto.getOnline_amount()));
				 ...
				 * if(i2!=0) {
				 * 
				 * // insert customer acc table info PreparedStatement cust_acc = db.connection
				 * .prepareStatement( "INSERT INTO customer_account_tb \r\n" +
				 ...
				 * ")VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);", Statement.RETURN_GENERATED_KEYS);
				 * 
				 * cust_acc.setInt(1, dto.getCustomer_id_fk()); cust_acc.setInt(2,
				 * dto.getUser_id_fk()); cust_acc.setInt(3, dto.getId()); cust_acc.setString(4,
				 * dto.getC_y_session()); cust_acc.setFloat(5, dto.getFinal_amount());//debit
				 * amount cust_acc.setFloat(6, dto.getCash_amount()+dto.getOnline_amount()); //
				 * credit cust_acc.setString(7, "Sale Bill"); cust_acc.setFloat(8,
				 * dto.getCash_amount()); cust_acc.setFloat(9, dto.getOnline_amount());
				 * cust_acc.setString(10, dto.getOnline_way()); cust_acc.setString(11,
				 * dto.getOnline_remark()); cust_acc.setString(12, dto.getOnline_date());
				 * cust_acc.setString(13, dto.getPayment_mode()); cust_acc.setString(14,
				 * dto.getBill_date()); cust_acc.setString(15, dto.getStatus());
				 * 
				 * 
				 ...
				 * 
				 * ResultSet rs_cust_acc = cust_acc.getGeneratedKeys(); rs_cust_acc.next();
			...
			//*************** update cash and payment id in sale_bill_tb *****************
						PreparedStatement update_sale_bill = db.connection
								.prepareStatement(
										"UPDATE sale_bill_return_tb SET\n"
										+ "	cash_payment_id_fk = ? , \n"
		...
		public ArrayList<SaleBillDto> getSaleBillReturnInfo(String d1,String d2,String all,ServletConfig config, HttpServletRequest request)
				throws IOException {
			...
			try {
				...
				if (all.equalsIgnoreCase("Yes")) {
					...
					preparedStatement = db.connection.prepareStatement("SELECT 	s.id, s.invoice_no,s.customer_id_fk,s.c_y_session,s.total_basic_amt,s.total_gst_amt,s.total_amt_with_gst,s.discount_per,s.discount_amount,s.final_amount,\n"
							...
							+ "	s.gst_status,s.remark,s.igst_status,s.status,s.bank_id_fk,"
							+ " c.name, c.mobile_no, c.address, c.gst_no ,c.status"
							+ "	FROM sale_bill_return_tb s \n"
							+ "	INNER JOIN user_personal_info_tb up ON up.id=s.user_id_fk \n"
							+ "	INNER JOIN customer_info_tb c ON c.id=s.customer_id_fk \n"
				...
				while (resultSet.next()) {
					...
					dto.setStatus(resultSet.getString(33));
					dto.setBank_id_fk(resultSet.getInt(34));
					dto.setCust_name(resultSet.getString(35));
					dto.setCust_mobile_no(resultSet.getString(36));
					dto.setCust_address(resultSet.getString(37));
					dto.setCust_gst_no(resultSet.getString(38));
					dto.setCust_status(resultSet.getString(39));
		...
		public ArrayList<SaleBillItemDto> getSellBillItemInfoReturn(String d1, String d2, String item_name,String all , ServletConfig config, HttpServletRequest request)
				throws IOException {
			...
			try {
				...
				if (all.equalsIgnoreCase("Yes")) {
					preparedStatement = db.connection.prepareStatement(
							...
							+ "it.sell_item_status, it.current_in_date , ct.name,bt.bill_date\r\n"
							+ "FROM sale_bill_item_return_tb it INNER JOIN sale_bill_return_tb bt ON it.bill_id_fk = bt.id\r\n"
							+ "INNER JOIN customer_info_tb ct ON bt.customer_id_fk = ct.id ;");
					...
					preparedStatement = db.connection.prepareStatement("SELECT 	it.id, it.bill_id_fk, it.cat_id_fk, it.cat_name, it.measurement_id_fk, it.measurement_name, it.item_code, it.item_id_fk, it.item_name, it.item_qty, it.sell_base_price, \r\n"
							+ "it.discount_sell_gst_price, it.discount_per, it.discount_per_amount, it.gst_per, it.hsn_code, it.sell_item_status, it.current_in_date , ct.name,bt.bill_date\r\n"
							+ "FROM sale_bill_item_return_tb it INNER JOIN sale_bill_return_tb bt ON it.bill_id_fk = bt.id\r\n"
							+ "INNER JOIN customer_info_tb ct ON bt.customer_id_fk = ct.id \r\n"
					...
					preparedStatement = db.connection.prepareStatement(
							...
							+ "it.sell_item_status, it.current_in_date , ct.name,bt.bill_date\r\n"
							+ "FROM sale_bill_item_return_tb it INNER JOIN sale_bill_return_tb bt ON it.bill_id_fk = bt.id\r\n"
							+ "INNER JOIN customer_info_tb ct ON bt.customer_id_fk = ct.id WHERE bt.bill_date=CURDATE();");
				}
		...
		// Method for Show data on Manage page
				public ArrayList<SaleBillDto> getSaleBillReturnInfoByCust(int cust_id,String d1,String d2,ServletConfig config, HttpServletRequest request)
						throws IOException {
					...
					try {
						...
						if (d1.equalsIgnoreCase("") && d2.equalsIgnoreCase("")) {
							...
							preparedStatement = db.connection.prepareStatement("SELECT 	s.id, s.invoice_no,s.customer_id_fk,s.c_y_session,s.total_basic_amt,s.total_gst_amt,s.total_amt_with_gst,s.discount_per,s.discount_amount,s.final_amount,\n"
									...
									+ "	INNER JOIN customer_info_tb c ON c.id=s.customer_id_fk \n"
									+ "	LEFT JOIN bank_tb bt ON bt.id=s.bank_id_fk \n"
									+ "	WHERE s.bill_date BETWEEN ? AND NOW()"
									+ "	AND s.customer_id_fk = ?;");
							...
							preparedStatement.setString(1, d1);
							preparedStatement.setString(2, d2);
							preparedStatement.setInt(3, cust_id);
						}
		...
		// Method for Show data on edit page
				public SaleBillDto getSaleBillReturnInfoById(int id,ServletConfig config, HttpServletRequest request)
						throws IOException {
					...
					try {
						...
						preparedStatement = db.connection.prepareStatement("SELECT 	s.id, s.invoice_no,s.customer_id_fk,s.c_y_session,s.total_basic_amt,s.total_gst_amt,s.total_amt_with_gst,s.discount_per,s.discount_amount,s.final_amount,\r\n"
								+ "s.paid_amount,s.taxable_value_0,s.gst_amount_5,s.taxable_value_5,s.gst_amount_12,s.taxable_value_12,s.gst_amount_18,s.taxable_value_18,s.gst_amount_28, \r\n"
								+ "	s.taxable_value_28,s.user_id_fk,s.bill_date,s.current_in_date,s.payment_mode,s.online_amount,s.cash_amount,s.online_date,s.online_remark,s.online_way,s.gst_status,s.remark,s.igst_status,s.status,s.bank_id_fk,\r\n"
								+ "	c.name, c.mobile_no, c.address, c.gst_no ,c.status,bt.name,bt.ifsc_code,bt.account_no\r\n"
								+ "	FROM sale_bill_return_tb s INNER JOIN user_personal_info_tb up ON up.id=s.user_id_fk INNER JOIN customer_info_tb c ON c.id=s.customer_id_fk\r\n"
...
public ArrayList<SaleBillItemDto> getSaleBIllItemReturnInfoByBillId(int bill_id,ServletConfig config, HttpServletRequest request) throws IOException{
					...
					try {
						...
						preparedStatement = db.connection.prepareStatement("SELECT 	id, bill_id_fk,cat_id_fk,cat_name,"
								...
								+ "	sell_item_status,current_in_date "
								+ "	FROM sale_bill_item_return_tb WHERE bill_id_fk=?;");
		...
		public boolean insertSaleBillItemReturn(
				...
				String[] Item_name,String[] Item_code, String[] Qty,  String[] Tot_amt,String[]Dis_per,String[]Dis_amt,String[]Discount_Sale_gst_price,String[]Sale_item_status, SaleBillDto pur_dto, int bill_id_fk,
			...
			try {
				...
				for (int i = 0; i < Item_name.length; i++) {
					...
					String sql = "\r\n"
							+ "INSERT INTO sale_bill_item_return_tb (bill_id_fk,cat_id_fk,cat_name, \r\n"
							...
							+ "discount_per_amount,gst_per,hsn_code,sell_item_status)\r\n"
							+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);\r\n"
							+ "";
		...
		// Method For insert in Sale bill item 
				public boolean insertOneSaleBillItemReturn(SaleBillItemDto dto,
						HttpServletRequest request, ServletConfig config)
						throws IOException {
					DataDb comm = new DataDb(request);
					...
					try {
							...
							String sql = " INSERT INTO sale_bill_item_return_tb (bill_id_fk,cat_id_fk,cat_name, \r\n"
									+ "	measurement_id_fk,measurement_name,item_code,item_id_fk,item_name,item_qty,sell_base_price,discount_sell_gst_price,discount_per, \r\n"
									+ "	discount_per_amount,gst_per,hsn_code,sell_item_status)\r\n"
									+ "	VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
				...
				public boolean deleteOneSaleBillItemReturn(int bill_item_id, HttpServletRequest request, ServletConfig config)
						throws IOException {
					...
					try {
						...
						PreparedStatement select_item_det = comm.connection
								.prepareStatement("SELECT item_qty,item_id_fk FROM sale_bill_item_return_tb"
										+ "	WHERE id = ?;");
						...
						while (rs_item.next()) {
							...
							String sql = "DELETE FROM sale_bill_item_return_tb WHERE id = ?;";
		...
		public boolean updateSaleBillReturnInfoById(SaleBillDto dto, PaymentDto pay_dto, HttpServletRequest request,
			...
			int old_customer_id_fk=0, new_cash_id = 0,new_online_id = 0;

			try {

				PreparedStatement ps1 = db.connection
						.prepareStatement("SELECT 	customer_id_fk, final_amount, \r\n"
								+ "	paid_amount,payment_mode,cash_payment_id_fk, \r\n"
								+ "	online_payment_id_fk FROM sale_bill_return_tb"
								+ "	WHERE id = ?;");
				...
				while (rs1.next()) {
					...
					dto.setCash_payment_id_fk(rs1.getInt(5));
					dto.setOnline_payment_id_fk(rs1.getInt(6));

				}
				
				/*
				 * // clear customer old due info PreparedStatement old_cust_due = db.connection
				 * .prepareStatement(
				 * "UPDATE customer_info_tb SET old_due = old_due - ? WHERE id = ?;");
				 * 
				...
				// ****** when old customer & New customer is Different ********
				if (old_customer_id_fk != dto.getCustomer_id_fk()) {
					
					// when customer is new in record
				...
				/*
				 * // update customer due info PreparedStatement cust_due = db.connection
				 ...
				 * 
				 * // insert customer acc table info PreparedStatement cust_acc = db.connection
				 * .prepareStatement( "UPDATE customer_account_tb SET\r\n" +
				 * "	customer_id_fk = ?,user_id_fk = ?,bill_id_fk = ?,\r\n" +
				 * "	c_y_session = ?,debit_amount = ?,credit_amount = ?,TYPE = ?, \r\n" +
				 * "	cash_amount = ?,online_amount = ?,online_way = ?,online_remark = ?,\r\n"
				 * + "	online_date = ?,payment_mode = ?,pay_date = ?,STATUS = ?\r\n" +
				 ...
				 * cust_acc.setString(13, dto.getPayment_mode()); cust_acc.setString(14,
				 * dto.getBill_date()); cust_acc.setString(15, dto.getStatus());
				 * cust_acc.setInt(16, dto.getCustomer_account_id_fk());
				 * 
				 * 
				 * 
				 * System.out.println(cust_acc);
				...
				ps = db.connection.prepareStatement(
						...
						+ "remark = ?,igst_status = ?,STATUS = ?,\r\n"
						+ "bank_id_fk = ?,online_payment_id_fk = ?,cash_payment_id_fk =? WHERE id = ?;");
				...
				ps.setString(31, dto.getStatus());
				
				ps.setInt(32, dto.getBank_id_fk());
				ps.setInt(33, dto.getOnline_payment_id_fk());
				ps.setInt(34, dto.getCash_payment_id_fk());
				
				ps.setInt(35, dto.getId());
