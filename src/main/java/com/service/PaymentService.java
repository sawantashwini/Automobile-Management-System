	...
	// ******** Cash insert method Start
		public int insertCashPayment(PaymentDto dto,HttpServletRequest request, ServletConfig config) throws IOException {
			...
			try {
				...
				/*if(dto.getCash_amount()!=0||dto.getDebit_cash_amount()!=0) {*/
		...
		// ******** Online Cash Update method Start
				public boolean updateCashPayment(PaymentDto dto,int cash_id, HttpServletRequest request, ServletConfig config) throws IOException {
					...
					/*	if(cash_id==0&&(dto.getCash_amount()!=0||dto.getDebit_cash_amount()!=0)) {
						insertCashPayment( dto, request,  config);
						}
						else {*/
						PreparedStatement ps = db.connection.prepareStatement(
								"UPDATE cash_payment_tb SET \n"
				...
				// ******** Online Cash Delete method Start
							public boolean deleteCashPayment(int cash_id, HttpServletRequest request, ServletConfig config) throws IOException {
								...
								try {
									...
									PreparedStatement ps = db.connection.prepareStatement(
											"DELETE FROM cash_payment_tb WHERE id = ? ;");
		...
		public int insertOnlinePayment(PaymentDto dto,HttpServletRequest request, ServletConfig config) throws IOException {
			...
			try {
				...
				/*if(dto.getOnline_amount()!=0||dto.getDebit_online_amount()!=0) {*/
				PreparedStatement ps = db.connection.prepareStatement(
						"INSERT INTO online_payment_tb \n"
		...
		// ******** Online Payment Update method Start
			public boolean updateOnlinePayment(PaymentDto dto,int online_id ,HttpServletRequest request, ServletConfig config) throws IOException {
				...
				try {
					/*
					 * if(online_id==0&&(dto.getOnline_amount()!=0||dto.getDebit_online_amount()!=0)
					 * ) { insertCashPayment( dto, request, config); } else {
					 */
					PreparedStatement ps = db.connection.prepareStatement(
							"UPDATE online_payment_tb SET \n"
							+ "	bank_id_fk = ?,credit = ?,debit = ?,remark =  ?,  \n"
			...
			// ******** Online Payment Delete method Start
					public boolean deleteOnlinePayment(int online_id, HttpServletRequest request, ServletConfig config) throws IOException {
						...
						try {
							...
							PreparedStatement ps = db.connection.prepareStatement(
									"DELETE FROM online_payment_tb WHERE id = ? ;");
