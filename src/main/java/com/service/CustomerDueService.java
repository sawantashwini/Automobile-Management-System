		...
		public CustomerDto getCustomerDueInfoByPaidId(int id, ServletConfig config,
			...
			try {
				...
				preparedStatement = db.connection
						.prepareStatement("SELECT dt.id,user_id_fk, NAME, mobile_no, address, gst_no, old_due, opening_due, dt.status, \r\n"
								+ "pay_date, remark, payment_mode, online_way, online_remark, online_amount, cash_amount, online_date, bank_id_fk,upcoming_date,upcoming_remark\r\n"
								+ "FROM customer_due_tb dt INNER JOIN customer_info_tb it ON dt.customer_id_fk = it.id\r\n"
								+ "WHERE  dt.id= ?;\r\n"
								+ "");
				...
				while (resultSet.next()) {
					...
					dto.setUpcoming_date(resultSet.getString(19));
					dto.setUpcoming_remark(resultSet.getString(20));
