	...
	public EmployeeAccountDto getEmployeeAccountInfoById(int id, ServletConfig config, HttpServletRequest request)
		...
		try {
			...
			ps = db.connection.prepareStatement("SELECT ea.id,ea.employee_id_fk,ea.debit_amount, ea.credit_amount,"
					...
					+ "emp.mobile_no,emp.salary_per_month, usp.name, emp.qualification,emp.address, ea.in_date,ea.remark,ea.bank_id_fk FROM employee_account_tb ea INNER JOIN "
					+ "employee_personal_info_tb emp ON emp.id=ea.employee_id_fk INNER JOIN user_personal_info_tb "
					+ "usp ON usp.id=ea.user_id_fk WHERE ea.id = ?;");
