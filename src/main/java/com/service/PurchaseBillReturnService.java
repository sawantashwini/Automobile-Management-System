...
public class PurchaseBillReturnService {
	...
	public String getMaxInvoiceNo(String s, HttpServletRequest request, ServletConfig config) throws IOException {
		...
		try {
			...
			preparedStatement = comm.connection.prepareStatement(
					"SELECT MAX(CAST(SUBSTRING(invoice_no, 1, LENGTH(invoice_no)-0) AS UNSIGNED)) FROM purchase_bill_return_tb");
	...
	public int insertPurchaseBillReturnInfo(PurchaseBillDto dto, HttpServletRequest request, ServletConfig config)
			throws IOException {
		...
		try {
			...
			PreparedStatement ps = db.connection.prepareStatement("\r\n" + "INSERT INTO purchase_bill_return_tb \r\n"
					...
					+ "	gst_amount_28, taxable_value_28,igst_status,remark) VALUES (?,?,?,?, ?, ?,?, ?, ?, ?, ?,?, ?, ?,?, ?, ?, ?,?);");
	...
	public ArrayList<PurchaseBillDto> getPurchaseBillReturnInfo(ServletConfig config, HttpServletRequest request)
			throws IOException {
		...
		try {
			...
			preparedStatement = db.connection.prepareStatement(
					"SELECT 	pb.id , pb.dealer_id_fk,pb.invoice_no,pb.c_y_session,pb.total_basic_amount,pb.total_gst_amount, pb.total_amount_with_gst, \r\n"
							...
							+ " pb.gst_amount_18,pb.taxable_value_18,pb.gst_amount_28,pb.taxable_value_28,pb.igst_status,\r\n"
							+ " pb.status,pb.remark,pb.user_id_fk,dt.name,dt.mobile_no,dt.address,dt.gst_no FROM purchase_bill_return_tb pb \r\n"
							+ " INNER JOIN dealer_info_tb dt ON dt.id = pb.dealer_id_fk INNER JOIN user_personal_info_tb  \r\n"
							+ " ut ON ut.id = pb.user_id_fk  ;");
			...
			while (resultSet.next()) {
				...
				dto.setDealer_name(resultSet.getString(22));
				dto.setDealer_mobile(resultSet.getString(23));
				dto.setDealer_address(resultSet.getString(24));
				dto.setDealer_gst_no(resultSet.getString(25));
	...
	// date vice search
	public ArrayList<PurchaseBillDto> getPurchaseBillReturnInfo(String d1, String d2, String all, ServletConfig config, HttpServletRequest request)
			throws IOException {
		...
		try {
			...
			if (all.equalsIgnoreCase("Yes")) {
				...
				preparedStatement = db.connection.prepareStatement("SELECT 	pb.id , pb.dealer_id_fk,pb.invoice_no,pb.c_y_session,pb.total_basic_amount,pb.total_gst_amount, pb.total_amount_with_gst,\r\n"
						...
						+ "FROM purchase_bill_return_tb pb INNER JOIN dealer_info_tb dt ON dt.id = pb.dealer_id_fk \r\n"
			...
			else {
				{
					preparedStatement = db.connection.prepareStatement(
							"SELECT pb.id , pb.dealer_id_fk,pb.invoice_no,pb.c_y_session,pb.total_basic_amount,pb.total_gst_amount, pb.total_amount_with_gst, \r\n"
									...
									+ " pb.status,pb.remark,pb.user_id_fk,dt.name,dt.mobile_no,dt.address,dt.gst_no FROM purchase_bill_return_tb pb \r\n"
									+ " INNER JOIN dealer_info_tb dt ON dt.id = pb.dealer_id_fk INNER JOIN user_personal_info_tb  \r\n"
									+ " ut ON ut.id = pb.user_id_fk WHERE pb.bill_date=CURDATE();");
	...
	public ArrayList<PurchaseBillItemDto> getPurchaseBillReturnItemInfo(String d1, String d2, String item_name,String all ,ServletConfig config, HttpServletRequest request)
			throws IOException {
		...
		try {
			...
			if (all.equalsIgnoreCase("Yes")) {
				preparedStatement = db.connection.prepareStatement(
						...
						+ "FROM purchase_bill_item_return_tb it INNER JOIN purchase_bill_return_tb bt ON it.pur_bill_id_fk = bt.id\r\n"
				...
				preparedStatement = db.connection.prepareStatement("SELECT it.id, it.item_code, it.item_id_fk, it.item_name, it.cat_id_fk, it.cat_name, it.measure_id_fk, it.measure_name, it.item_qty, it.pur_base_price, it.discount_per, it.discount_amt, \r\n"
						...
						+ "dt.name,bt.bill_date FROM purchase_bill_item_return_tb it INNER JOIN purchase_bill_return_tb bt ON it.pur_bill_id_fk = bt.id INNER JOIN dealer_info_tb dt\r\n"
		...
		try {
			...
			preparedStatement = db.connection.prepareStatement(
					"SELECT 	pb.id , pb.dealer_id_fk,pb.invoice_no,pb.c_y_session,pb.total_basic_amount,pb.total_gst_amount, pb.total_amount_with_gst, \r\n"
							...
							+ " pb.status,pb.remark,pb.user_id_fk,dt.name,dt.mobile_no,dt.address,dt.gst_no FROM purchase_bill_return_tb pb \r\n"
							+ " INNER JOIN dealer_info_tb dt ON dt.id = pb.dealer_id_fk INNER JOIN user_personal_info_tb  \r\n"
							+ " ut ON ut.id = pb.user_id_fk  WHERE pb.dealer_id_fk=?;");
	...
	public PurchaseBillDto getPurchaseBillReturnInfoById(int id, ServletConfig config, HttpServletRequest request)
			throws IOException {
		...
		try {
			...
			preparedStatement = db.connection.prepareStatement(
					"SELECT 	pb.id , pb.dealer_id_fk,pb.invoice_no,pb.c_y_session,pb.total_basic_amount,pb.total_gst_amount, pb.total_amount_with_gst, \r\n"
							...
							+ " pb.status,pb.remark,pb.user_id_fk,dt.name,dt.mobile_no,dt.address,dt.gst_no FROM purchase_bill_return_tb pb \r\n"
							+ " INNER JOIN dealer_info_tb dt ON dt.id = pb.dealer_id_fk INNER JOIN user_personal_info_tb  \r\n"
							+ " ut ON ut.id = pb.user_id_fk   WHERE pb.id=?;");
	...
	public boolean insertPurchaseBillReturnItem(String[] Pur_base_price, String[] Item_id, String[] Cat_id,
		...
		try {
			...
			for (int i = 0; i < Item_name.length; i++) {
				...
				String sql = "INSERT INTO purchase_bill_item_return_tb \r\n"
	...
	public boolean insertOnePurchaseBillReturnItem(PurchaseBillItemDto dto,
		...
		try {
			...
			String sql = "INSERT INTO purchase_bill_item_return_tb (pur_bill_id_fk,cat_id_fk,cat_name,measure_id_fk,measure_name,item_code,\r\n"
	...
	public boolean deleteOnePurchaseBillReturnItem(int bill_item_id, HttpServletRequest request, ServletConfig config)
			throws IOException {
		...
		try {
			...
			PreparedStatement select_item_det = comm.connection
					.prepareStatement("SELECT item_qty,item_id_fk FROM purchase_bill_item_return_tb"
							+ "	WHERE id = ?;");
			...
			while (rs_item.next()) {
				...
				String sql = "DELETE FROM purchase_bill_item_return_tb WHERE id = ?;";
	...
	public ArrayList<PurchaseBillItemDto> getPurchaseBillReturnItemInfoById(int bill_id, ServletConfig config,
			HttpServletRequest request) throws IOException {
		...
		try {
			...
			preparedStatement = db.connection.prepareStatement(
					"SELECT 	id, item_code, item_id_fk, item_name, cat_id_fk, cat_name, measure_id_fk, measure_name, \r\n"
							...
							+ "	purchase_bill_item_return_tb WHERE pur_bill_id_fk = ?; ");
	...
	public ArrayList<PurchaseBillItemDto> getPurchaseBillReturnItemInfoByIdOne(int id, ServletConfig config,
			HttpServletRequest request) throws IOException {
		...
		try {
			...
			preparedStatement = db.connection.prepareStatement(
					"SELECT 	id, item_code, item_id_fk, item_name, cat_id_fk, cat_name, measure_id_fk, measure_name, \r\n"
							+ "item_qty, pur_base_price, discount_per, discount_amt, pur_discount_price, gst_per, \r\n"
							+ "item_hsn_code, pur_item_status FROM purchase_bill_item_return_tb WHERE pur_bill_id_fk = ?; ");
			preparedStatement.setInt(1, id);
			ResultSet resultSet = preparedStatement.executeQuery();
	...
	public boolean updatePurchaseBillReturnInfoById(PurchaseBillDto dto, HttpServletRequest request, ServletConfig config)
			throws IOException {
		...
		try {
			...
			ps = db.connection.prepareStatement("UPDATE purchase_bill_return_tb\r\n"
					...
					+ " taxable_value_18 = ?, gst_amount_28 = ?, taxable_value_28 = ?, igst_status = ?, STATUS = ?,  remark = ?, user_id_fk = ?\r\n"
					+ "WHERE id = ?;");
			...
			ps.setString(18, dto.getStatus());
			ps.setString(19, dto.getRemark());
			ps.setInt(20, dto.getUser_id_fk());

			ps.setInt(21, dto.getId());
