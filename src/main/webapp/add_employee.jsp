...
<body>
	...
	<main id="main" class="main">
		...
		<!-- ======= Breadcrumbs ======= -->
		<%@include file="breadcrumbs/employee.jsp"%>
		<!--  Breadcrumbs End-->
						...
						<div class="card-body">
							...
							<form action="EmployeeServlet" enctype="multipart/form-data"
								...
								<%-- <div class="col-md-5 d-flex justify-content-center">
									<div id="city_id_fk_refresh" class="col-md-12">
										<div class="control form-floating mb-3">
											<select class="select-box-search" id="city_id_fk"
												name="City_id_fk" aria-label="working post" required>
												<option value="0">Select Your City</option>
												<%
												CityService service = new CityService();
												ArrayList<CityDto> list = service.getActiveCityInfo(config, request);
												for (CityDto city_dto : list) {
												%>
												<option value="<%=city_dto.getId()%>"><%=city_dto.getName()%></option>
												<%
												}
												%>

											</select> <label for="city_id_fk">Select City
											</label>
											<div class="invalid-feedback">Please, Select Employee Living City!</div>
										</div>
									</div>
								</div>
