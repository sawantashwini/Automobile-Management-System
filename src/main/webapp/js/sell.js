...
var formated_warranty = convertDate(warranty);
      ...
      var newCell = newRow.insertCell(10);
		newCell.innerHTML = formated_warranty + '<input id="warranty' + preWorkExpIndex
			+ '" type="hidden" name="Warranty" value="' + warranty + '">';
		newCell.setAttribute("align", "center");
...
function manageRow(rowNo) {
	...
	for (var index = rowNo; index <= tblObj.rows.length; index++) {
		...
		var warranty = document.getElementById("warranty" + j).value;
...
var newCell = newRow.insertCell(10);
		newCell.innerHTML = formated_warranty + '<input id="warranty' + index
			+ '" type="hidden" name="Warranty" value="' + warranty + '">';
		newCell.setAttribute("align", "center");
	...
	if (name != "") {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				var check_cust = document.getElementById("check_cust").value;
				//alert(check);
				...
				if (check_cust == "done") {
					...
					document.getElementById('old_due').innerHTML = 'Old Due:'+ document
						.getElementById('old_due_val').value;

				document.getElementById('old_due').setAttribute('readonly', true);
...
function removeCustDet() {
	...
	document.getElementById('old_due').removeAttribute('readonly');
	...
	document.getElementById('old_due').innerHTML = "";
	document.getElementById('customer_id_fk').value = "0";
}

function removeCustDetEdit() {
...
/*function checkCustAvail(){
	...
	if (document.getElementById('regular').checked && customer_id_fk==0) {
...
function findDuplicateCode() {
	...
	for (var i = 1; i <= arr.length; i++) {
		...
		if (document.getElementById("item_code").value == document
			.getElementById("item_code" + i).value) {
...
function insertItem() {
	...
	if (item_code == "") {
		...
		$.ajax({
			...
			url : 'AjaxService/AjaxOneSellBillItem.jsp',
...
function checkPaymentMode1() {
			...
			//alert("hello");
					...
					if (document.getElementById('cash_mode').checked) {
						...
						document.getElementById("bank_id_fk").removeAttribute('required');
		document.getElementById("bank_id_fk").removeAttribute('name');
						...
						document.getElementById("remaining_amount").value = final_amount.toFixed(2);
							document.getElementById("bank_id_fk").setAttribute('required', '');
		document.getElementById("bank_id_fk").setAttribute('name', 'Bank_id_fk');
		...
		alert("Paid amount is greater than final amount.");
			document.getElementById("remaining_amount").value = Math.round(final_amount.toFixed(2));
				if(bill_id==0){
...
function checkCustomerIsRegular() {
//alert("hlo");
	    var customer_id_fk = document.getElementById("customer_id_fk").value;
	    ...
	    if(customer_id_fk > 0 || regular.checked){
	    	
	    }else  if (remaining_amount >=1) {
	        // The input is blank, prevent the form submission
	        alert("Customer is not regular pay full amount.");
	        event.preventDefault(); // Prevent form submission
	        
	        document.getElementById('cash_amount').focus();
	        document.getElementById('cash_amount').value = final_amount;
	    }
 ...
 function checkCustAvail() {
	...
	var customer_id_fk = document.getElementById("customer_id_fk").value;
	//alert(customer_id_fk);
	var regular = document.getElementById("regular");
	var flag = "Customer";

	if (regular.checked && customer_id_fk == 0) {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				var check_cust = document.getElementById("check").value;
				if (check_cust == 'true') {
					document.getElementById('name').value = "";
					//alert("done");
